4.1.5-p22 / 2025-04-05
==========================

 * Change: Acelle is now on Laravel 12, the #1 PHP framework
 * Change: PHP 8.2+ support, older versions of PHP might not work
 * Added: More trigger types for automation
 * Added: Official support for distributed deployment
 * Added: Official support for Supervisor: safe & faster delivery
 * Added: Support for remote Redis queues
 * Added: Better integration with local mailers (Postfix, Exim, etc.)
 * Added: Smart caching, improve web loading performance
 * Added: Improved plugin mechanism, easier to plug features in!
 * Added: Improved plugin mechanism, embrace AI plugins!
 * Added: Smart mapping of input file's fields and list fields
 * Added: Additional bulk operations support for list management
 * Added: More API support for administration tasks
 * Added: More comprehensive report for campaign delivery
 * Added: More detailed tracking information of automation
 * Added: More GDPR compliance
 * Fixed: Better payment integration with wallets like PayPal
 * Fixed: Reduced network bandwidth overhead in general
 * Fixed: Encoding issues with Japanese content
 * Fixed: Inconsistent measures in campaign delivery report
 * Fixed: HTML content and email client compatibility issues
