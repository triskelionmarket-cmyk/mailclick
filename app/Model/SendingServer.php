...
class SendingServer extends Model
{
    ...
    protected $fillable = [
        ...
        'smtp_password', 'smtp_port', 'smtp_protocol', 'quota_value', 'sendmail_path','twilio_account_sid', 'twilio_auth_token', 'twilio_from_number','quota_base', 'quota_unit',
    ...
    public static $serverMapping = array(
        ...
        self::TYPE_TWILIO => 'SendingServerTwilio',
    );
    ...
    /**
     * Send a test SMS for the sending server.
     */
    public function sendTestSms($toNumber, $content)
    {
        // Check if the current server supports sendSms (like Twilio)
        if (method_exists($this, 'sendSms')) {
            try {
                $result = $this->sendSms($toNumber, $content);
        ...
        return 'Error: sendSms method not implemented for this sending server type.';
    }
