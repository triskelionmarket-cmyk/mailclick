...
class Subscriber extends Model
{
    ...
    public function scopeByVerificationStatus($query, $verificationStatuses = [])
    {
        if (in_array(static::VERIFICATION_STATUS_UNVERIFIED, $verificationStatuses)) {
            // WHERE NULL must be specified explicitly
            return $query->where(function ($query) use ($verificationStatuses) {
                $query->whereNull('subscribers.verification_status')
                      ->orWhereIn('subscribers.verification_status', $verificationStatuses);
            });
        } else {
            return $query->whereIn('subscribers.verification_status', $verificationStatuses);
        }
    }
     ...
     */
    public function updateFields($params, $updateAll = true)
    {
    ...
    public function getFullName($default = null, $reload = true, $tagsToValueCache = [])
    {
        if ($reload && $this->id) {
            $this->refresh();
        }
        ...
        $first = $tagsToValueCache['FIRST_NAME'] ?? $this->getValueByTag('FIRST_NAME');
        $last = $tagsToValueCache['LAST_NAME'] ?? $this->getValueByTag('LAST_NAME');
    ...
    public static function removeSubscribersTags($subscribers, $tags)
    {
        // make validator
        $validator = \Validator::make([
            'tags' => $tags,
    ...
    public function getAllTagValues()
    {
        $tags = [];

        $tags['LIST_UID'] = $this->mailList->uid;
        $tags['LIST_NAME'] = $this->mailList->name;
        $tags['LIST_FROM_NAME'] = $this->mailList->from_name;
        $tags['LIST_FROM_EMAIL'] = $this->mailList->from_email;
        $tags['SUBSCRIBER_UID'] = $this->uid;

        # Subscriber custom fields
        foreach ($this->mailList->fields as $field) {
            $tags['SUBSCRIBER_'.$field->tag] = $this->getValueByField($field);
            $tags[$field->tag] = $this->getValueByField($field);
        }

        // Hack
        $tags['NAME'] = $this->getFullName(null, false, $tags);
        $tags['FULL_NAME'] = $this->getFullName(null, false, $tags);
