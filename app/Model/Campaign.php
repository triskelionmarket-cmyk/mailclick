...
class Campaign extends BaseCampaign implements HasTemplateInterface, CampaignInterface
{
    ...
    public function trackMessage($response, $subscriber, $server, $msgId)
    {
        ...
        if (!isset($params['runtime_message_id'])) {
            ...
            Customer::mapKeyToCustomer('runtime_message_id', $params['runtime_message_id'], $this->customer->id);
        }

        // create tracking log for message
        $result = $this->setConnection($this->customer->getDbConnection())->trackingLogs()->create($params);
    ...
    public function step()
    {
        ...
        // Step 2
        if (!empty($this->name) && !empty($this->subject) &&
            !empty($this->from_name) &&
            ($this->use_default_sending_server_from_email || (!empty($this->from_email) && !empty($this->reply_to)))
        ) {
            $step = 2;
        } else {
            return $step;
    ...
    public function isStageExcluded(string $name): bool
    {
        switch ($name) {
            ...
            case TransformUrl::class:
                 ...
                 *         return true;
                 *     }
                 * Cannot skip TransformUrl, otherwise, links like "/assets/..." will not be transformed to a public form
                 * So, always return false (do not exclude) here
                 *
                */
                return false;
    ...
    public function setContentAsTemplate($content)
    {
        ...
        $template->is_private = Template::IS_PRIVATE;
