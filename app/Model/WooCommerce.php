...
class WooCommerce extends Source
{
    ...
    public function importProducts()
    {
        ...
        foreach ($products as $key => $product) {
            try {
                // Căutăm produsul după source_item_id și source_id
                $p = Product::where('source_item_id', $product["id"])
                ...
                if (!$p) {
                    ...
                    $p = new Product();
                    $p->source_item_id = $product["id"];
                    $p->source_id = $this->id;
                    $p->customer_id = $this->customer_id;
                }

                // Actualizăm datele produsului (existent sau nou)
