        ...
        return StringHelper::updateHtml($html, function ($dom) {
            ...
            foreach ($nodes as $node) {
                try {
                    ...
                    $configdata = $node->getAttribute('data-config');
                    $configdata = base64_decode($configdata);

                    // IMPORTANT, otherwise json_decode might return null;
                    $configdata = mb_convert_encoding($configdata, 'UTF-8', 'UTF-8');

                    $config = json_decode($configdata, true);

                    if (is_null($config)) {
                        throw new \Exception('Cannot parse RSS configuration settings');
                    }
