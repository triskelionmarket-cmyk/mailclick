...
@section('content')
    ...
    <div class="d-flex">
        <div class="me-auto pe-5">
            <h2>{{ trans('messages.sending_credit.overview') }}</h2>
            <p>{{ trans('messages.sending_credit_plan.wording') }}</p>
        </div>
        <div class="text-end d-none">
            <a data-action="buy-credit"
                href="{{ action("SendingCreditPlanController@select") }}" role="button"
                class="btn btn-primary text-nowrap"
            >
                <span class="material-symbols-rounded">add</span> {{ trans('messages.sending_credit_plan.buy_more') }}
            </a>
        </div>
    </div>
    ...
    <div class="stats-boxes">
        <div class="stats-box px-4" style="width:33.3%">
            ...
            {{-- <p>{{ trans('messages.sending_credit_plan.credits') }}</p> --}}
            <div class="text-start">
                <a data-action="buy-credit"
                    href="{{ action("SendingCreditPlanController@select") }}" role="button"
                    class="btn btn-primary text-nowrap mt-2"
                >
                    <span class="material-symbols-rounded me-1">add</span> {{ trans('messages.sending_credit_plan.buy_more') }}
                </a>
            </div>
        </div>
        <div class="stats-box px-4" style="width:33.3%">
            @if (Auth::user()->customer->getLastPaidSendingCreditsInvoice())
                <h3>
                    <span class="material-symbols-rounded text-success me-2">
                        add_task
                    </span>
                    {{ number_with_delimiter(number_with_delimiter(Auth::user()->customer->getLastPaidSendingCreditsInvoice()->sending_credits)) }}
                    {{-- {{ trans('messages.sending_credit.count', [
                        'number' => number_with_delimiter(number_with_delimiter(Auth::user()->customer->getLastPaidSendingCreditsInvoice()->sending_credits))
                    ]) }} --}}
                </h3>
                <p class="mb-0">
                ...
                <p>
                    (
                        <span class="text-semibold">{{ format_price(Auth::user()->customer->getLastPaidSendingCreditsInvoice()->total(), Auth::user()->customer->getLastPaidSendingCreditsInvoice()->currency->format) }}</span>
                        ;
                    ...
                    <button type="button" class="btn btn-light sort-direction" data-popup="tooltip" title="{{ trans('messages.change_sort_direction') }}" role="button" class="btn btn-xs">
                        ...
                        <script>
                            var InvoiceList = {
                                getList: function() {
                                    return makeList({
                                        url: '{{ action('SendingCreditPlanController@invoiceList', [
                                            'show_customer' => true,
                                        ]) }}',
                    ...
                    <button type="button" class="btn btn-light sort-direction" data-popup="tooltip" title="{{ trans('messages.change_sort_direction') }}" role="button" class="btn btn-xs">
                        ...
                        <script>
                            var TransactionList = {
                                getList: function() {
                                    return makeList({
                                        url: '{{ action('SendingCreditPlanController@transactionList') }}',
