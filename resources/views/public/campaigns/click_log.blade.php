...
<form class="listing-form"
        data-url="{{ action('Pub\CampaignController@clickLogListing', [ 'uid' => $campaign->uid, 'customer_uid' => $campaign->customer->uid ]) }}"
...
<button type="button" class="btn btn-light sort-direction" data-popup="tooltip" title="{{ trans('messages.change_sort_direction') }}" role="button" class="btn btn-xs">
    ...
    <script>
        var CampaignLogIndex = {
            getList: function() {
                return makeList({
                    url: '{{ action('Pub\CampaignController@clickLogListing', [ 'uid' => $campaign->uid, 'customer_uid' => $campaign->customer->uid ]) }}',
