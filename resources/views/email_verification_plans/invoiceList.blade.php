			...
			<tr>
                ...
                <td>
                    <span class="no-margin kq_search font-weight-semibold" style="text-transform: capitalize;">
                        {{ number_with_delimiter($invoice->email_verification_credits) }}
                    </span>
                    <div>
                        <span class="text-muted2">{{ trans('messages.email_verification_plan.credits') }}</span>
                    </div>
                </td>
                <td>
					...
					<div class="btn-group">
                        @if ($invoice->isNew() && !$invoice->getPendingTransaction())
                            <a href="{{ action('CheckoutController@billingAddress', [
                                ...
                                {{ trans('messages.email_verification_plan.checkout') }}
                            </a>
                        @endif
